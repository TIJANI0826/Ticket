...
LOGIN_INVALID_MESSAGE = 'Invalid login details'
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        laundry = Laundry.objects.filter(user=self.request.user)
        ctx = super(HomeView, self).get_context_data(**kwargs)
        ctx['laundry'] = laundry


        halls = Package.objects.all()
        ctx['halls'] = halls
...
class Print(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        laundry = Laundry.objects.filter(user=self.request.user)
        ctx = super(Print, self).get_context_data(**kwargs)
        ctx['laundry'] = laundry
...
class HallView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        halls = Package.objects.filter(availability=True)
        ctx['halls'] = halls
...
class HallBookingView(CreateView):
    ...
    def get_halls(self):
        ...
        halls = Package.objects.get(pk=hall_pk)
...
class LaundryDetails(DetailView):
    template_name = 'event/eventsdetails.html'
    model = Package

    def get_context_data(self, **kwargs):
        laundry = Laundry.objects.filter(user=self.request.user)
        ctx = super(LaundryDetails, self).get_context_data(**kwargs)
        ctx['booking_success'] = 'booking-success' in self.request.GET
        ctx['laundry'] = laundry
